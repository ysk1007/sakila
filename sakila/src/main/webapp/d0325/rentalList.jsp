...
<%
	...
	sql = "SELECT COUNT(*) count FROM"
		+ " (SELECT t.*, i.film_id, i.store_id FROM "
		+ " 		(SELECT r.inventory_id, r.rental_id,CONCAT('(',CONCAT(c.first_name,' ',c.last_name),')'"
		+ " 		,c.customer_id) AS NAME, r.rental_date, r.return_date "
		+ " 		FROM rental r "
		+ " 		INNER JOIN customer c ON r.customer_id = c.customer_id) t"
		+ " 	INNER JOIN inventory i ON t.inventory_id = i.inventory_id) t2"
		+ " INNER JOIN film f ON t2.film_id = f.film_id"
		+ " WHERE " + where;
	...
	// 테이블 쿼리
	sql = "SELECT t2.*, f.title title FROM"
		+"(SELECT t.*, i.film_id, i.store_id FROM "
		+"		(SELECT r.inventory_id, r.rental_id,CONCAT(CONCAT(c.first_name,' ',c.last_name),'('"
		+"		,c.customer_id,')') AS NAME, r.rental_date, r.return_date "
		+"		FROM rental r "
...
<body>
	...
	<div>
		...
		<a href="<%=jsp%>
		?currentPage=1
		&title=<%=searchWord.equals("") ? "" : searchWord%>
		&storeId=<%=storeId%>">[처음]</a>
		
		<!-- [이전 10] -->
		<a href="<%=jsp%>
		?currentPage=<%=currentPage - 10%>
		&title=<%=searchWord.equals("") ? "" : searchWord%>
		&storeId=<%=storeId%>">[이전 10]</a>
		...
		?currentPage=<%=currentPage + 10%>
		&title=<%=searchWord.equals("") ? "" : searchWord%>
		&storeId=<%=storeId%>">[다음 10]</a>
		
		<!-- [마지막] -->
		<a href="<%=jsp%>
		?currentPage=<%=lastPage%>
		&title=<%=searchWord.equals("") ? "" : searchWord%>
		&storeId=<%=storeId%>">[마지막]</a>
	</div>
	...
	<form action="/sakila/d0325/rentalList.jsp">
		...
		영화 제목 : <input type="text" name="title" value="<%=searchWord.equals("") ? "" : searchWord%>">
