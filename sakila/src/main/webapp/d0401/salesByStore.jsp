...
<% 
	...
	String jsp = "/sakila/d0401/salesByStore.jsp";
	String[] col = {"store","manager","total_sales"};
	...
	// 데이터 개수 구하는 쿼리
	String sql = "SELECT COUNT(*) count FROM store";
	...
	// 테이블 출력 쿼리
	sql = "SELECT"
			+" CONCAT(ct.city,',',cn.country) AS 'store',"
...
<body>
	...
	<div>
		...
		<!-- [이전 10] -->
		<a href="<%=jsp%>?currentPage=<%=currentPage - 10%>">이전 10</a>
		...
		<%
			...
			for(int i = 1 ; i <= pageDiv ; i++){
				...
				if(p > lastPage) continue; // 마지막 페이지 크기보다 크면 생략
					%><a href="<%=jsp%>?currentPage=<%=p%>" class="<%=currentPage == i ? "selected" : "" %>"><%=p%></a><%
			}
		%>
		
		<!-- [다음 10] -->
		<a href="<%=jsp%>?currentPage=<%=currentPage + 10%>">다음 10</a>
